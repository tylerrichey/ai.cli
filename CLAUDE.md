...
1. **AiApplication** (`AiApplication.cs`) — owns CLI parsing via `System.CommandLine`. Handles `--version` as a fast path before parsing. Routes to either model listing or command generation. Copies generated output to clipboard.
...
7. **Output** (`Output/`) — `ShellCommandFormatter.FormatForOutput` handles all shell targets: PowerShell commands are trimmed, bash/zsh commands are wrapped in `<shell> -lc "..."`. `SystemClipboardService` wraps TextCopy.
