...
1. **AiApplication** (`AiApplication.cs`) — owns CLI parsing via `System.CommandLine`. Handles `--version` as a fast path before parsing. Routes to history search (`-hs`/`--history`), model listing, resume (`-r`/`--resume`), question answering (`-q`), or command generation. With `--execute`/`-x`, displays the generated command on stderr, prompts for Enter-to-confirm, and runs it via `ICommandExecutor`; otherwise generated commands are copied to the clipboard. Question and resume answers print to stdout only. Records each successful invocation to `IHistoryService` unless `--no-history`/`-nh` is set.
